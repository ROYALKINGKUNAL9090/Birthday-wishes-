    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Happy Birthday ${profile.name}! ðŸŽ‰`,
          text: `Check out this special birthday celebration page for ${profile.name}! ðŸŽ‚âœ¨`,
          url: fullUrl
        });
      } catch (e) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="relative w-full max-w-md bg-slate-900 border border-white/20 rounded-3xl p-6 shadow-2xl text-slate-100">
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/10">
              <a
                href={`https://api.whatsapp.com/send?text=${encodeURIComponent(`Check out ${profile.name}'s Birthday Celebration! ðŸŽ‰ ${shareUrl}`)}`}
                target="_blank"
